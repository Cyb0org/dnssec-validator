...
short textkey = 1;
short choice = 0;
char* dnssecseradr = "127.0.0.1";

wchar_t* DNSSECtext2 = L"";
// key state memory
int state;
// url address memory
char * urladdr = " ";
...
bool CKBBarBand::CreateToolWindow()
{
	...
	return false;
	//if (!SendMessage(m_wndToolBar, TB_SETEXTENDEDSTYLE, 0, TBSTYLE_EX_DRAWDDARROWS))
	//return false;
	//if (!SendMessage(m_wndToolBar, TB_SETBUTTONSIZE, 0, MAKELPARAM(ICON_KEY_WIDTH+200, ICON_KEY_HEIGHT)))
	//return false;
	//CreateStatusBarKey();
...
{   
	...
	//ldicon2 = IDI_ICON_KEY_GREY1; 	
	//statldicon = IDI_ICON_KEY_GREY; // action icon
	//RefreshIcon2();
	//ATLTRACE("IDI_ICON_KEY_ACTION call\n");
	//LoadOptions();
	...
	//result = NPAPI_EXIT_CONNECTION_DOMAIN_SECURED;
	//ATLTRACE("Critical section end\n");
...
char* CKBBarBand::URL2domain(char *url) 
{
	...
	namedomain=(char*)malloc(2083*sizeof(char));
...
/**************************************************************************/
void CKBBarBand::LoadOptions(void) {
	ATLTRACE("LoadOptions() call\n");
	...
	dwRet = RegOpenKeyEx(HKEY_CURRENT_USER, DSV_REG_KEY, 0, KEY_QUERY_VALUE, &hKey);
	if (dwRet == ERROR_SUCCESS) {

		ATLTRACE("ERROR_SUCCESS() call\n");
		// Get the registry values...
		HRESULT hr = RegGetString(hKey,"dnsserveraddr", &szVal);
		if (FAILED(hr)) return;
		dnssecseradr=(char*)szVal;
		ATLTRACE(dnssecseradr);

		hr = RegGetString(hKey,"nic", &szVal);
		if (FAILED(hr)) return;

		hr = RegGetString(hKey,"ora", &szVal);
		if (FAILED(hr)) return;
		...
		if (dwVal==0x00000000) choice = 0;
		else if (dwVal==0x00000001) choice = 1;
		else choice = 2;

		hr = RegGetDWord(hKey,"texts", &dwVal);
		if (FAILED(hr)) return;

		if (dwVal==0x00000001) textkey = 1;
		else textkey = 0;
		...
		dwSize = MAX_STR_LEN;
		// read preferences
		dwRet = RegQueryValueExA(hKey, "dnsserveraddr", NULL, NULL, (BYTE*)texts, &dwSize);
		if (dwRet != ERROR_SUCCESS) ATLTRACE("\nmam1\n");
		dwRet = RegQueryValueExA(hKey, "choice", NULL, NULL, (BYTE*)&prefs.dwDebugoutput, &dwSize);
		if (dwRet != ERROR_SUCCESS) ATLTRACE("\nmam2\n");
